...
def menuEsquerda(self):
    ...
    self.ui.botao_python.clicked.connect(lambda: paginaPython(self,0))
    self.ui.botao_python2.clicked.connect(lambda: paginaPython(self,2))
    self.ui.botao_python3.clicked.connect(lambda: paginaPython(self,3))
    self.ui.botao_python4.clicked.connect(lambda: paginaPython(self,4))
    self.ui.botao_python5.clicked.connect(lambda: paginaPython(self,5))
    self.ui.botao_evaluate.clicked.connect(lambda: evaluateModels(self))
...
#homepage ------------------------->
def paginaHome(self):
    self.ui.status.setText('home page')
    self.ui.status_pagina_atual.setText('home')
    self.ui.stackedWidget.setCurrentIndex(0)
    self.ui.rodape.hide()
    self.ui.path="images/bg_home.jpg"
    self.ui.background.setPixmap(QtGui.QPixmap("images/bg_home.jpg"))
#browser--------------------------->
def paginaBrowser(self):
    self.ui.status_pagina_atual.setText('Upload Signature')
    ...
    image_file, _ = QFileDialog.getOpenFileName(self, "Open Image", 
                ...
                GIF Files (*.gif)")
    if image_file != '':
        self.ui.path=image_file
        self.ui.background.setPixmap(QtGui.QPixmap(image_file))
    self.ui.stackedWidget.setCurrentIndex(0)
...
def paginaPython(self,id):
    if (self.ui.path!="images/bg_home.jpg"):
        ...
        if(id==0):
            ...
            dialog.setWindowTitle("Résultat Obtenu")
            text = "La signature prédite par la méthode de K les plus proches voisins est  : "
            text = text + rs[0]
            dialog.setText(text)         
            dialog.show()
            ...
            dialog = QMessageBox(self)
            dialog.setWindowTitle("Résultat Obtenu")
            text = "La signature prédite par la méthode de Naive Bayes est  : "
            text = text + rs[0]
            dialog.setText(text)         
            dialog.show()
        elif(id ==3) :
            ...
            dialog = QMessageBox(self)
            dialog.setWindowTitle("Résultat Obtenu")
            text = "La signature prédite par la méthode de Support à vecteur machine est  : "
            text = text + rs[0]
            dialog.setText(text)         
            dialog.show()
        elif(id == 4) :
            ...
            dialog = QMessageBox(self)
            dialog.setWindowTitle("Résultat Obtenu")
            text = "La signature prédite par la méthode d'arbre de décision est  : "
            text = text + rs[0]
            dialog.setText(text)         
            dialog.show()
        elif(id==5):
            ...
            dialog = QMessageBox(self)
            dialog.setWindowTitle("Résultat Obtenu")
            text = "La signature prédite par la méthode de foret aléatoire est  : "
            text = text + rs[0]
            dialog.setText(text)         
            dialog.show()
        ...
        dialog.setText("Aucune image n'a été télécharger. Veuillez ouvrir une image avant de commencer le traitement ")          
        dialog.show()
...
def evaluateModels(self):
    ...
    print(rs)
    self.ui.stackedWidget.setCurrentIndex(7)
    text="le taux de precision est de "
    knn = str(rs[0])
    gnb = str(rs[1])
    svm = str(rs[2])
    dt = str(rs[3])
    fr= str(rs[4])
    self.ui.status.setText('Python Interpreter | Evaluation')
    self.ui.status_pagina_atual.setText('python')
    
    self.ui.input_site.setText(text+knn)
    self.ui.input_site_gnb.setText(text+gnb)
    self.ui.input_site_svm.setText(text+svm)
    self.ui.input_site_dt.setText(text+dt)
    self.ui.input_site_fr.setText(text+fr)
...
#     self.ui.status_pagina_atual.setText('methods')
#     self.ui.stackedWidget.setCurrentIndex(1)
#     self.ui.rodape.show()
# def abreMenuCamera(self):
#     self.ui.texto_menu_camera1.setText("methods")
